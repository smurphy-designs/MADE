...
float minDepth = 500 / 304.8; // Minimum depth threshold in feet
float maxDepth = 2000 / 304.8; // Maximum depth threshold in feet
int rows = 9;
int cols = 8;
...
void draw() {
  ...
  minDepth = minDepthSlider.getValue() / 304.8; // Convert minDepth to feet
  maxDepth = maxDepthSlider.getValue() / 304.8; // Convert maxDepth to feet
  ...
  text(nf(minDepthSlider.getValue() / 304.8, 1, 2) + " ft", 270, 560);

  maxDepthSlider.display("Max Depth");
  text(nf(maxDepthSlider.getValue() / 304.8, 1, 2) + " ft", 270, 660);
...
void displayGridAndSendSerial(int[] rawDepth, int[] rawBodyTrack) {
  ...
  for (int row = 0; row < rows; row++) {
    ...
    for (int col = 0; col < cols; col++) {
      ...
      float avgDepth = getTileDepth(rawDepth, x1, y1, tileWidth, tileHeight) / 304.8; // Convert to feet
